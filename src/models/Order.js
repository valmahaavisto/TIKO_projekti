...
const getAllOrders = async () => {
    try {
        const result = await pool.query('SELECT * FROM BookOrder ORDER BY order_id');
...
const getOrderById = async (order_id) => {
   try {
        const result = await pool.query('SELECT * FROM BookOrder WHERE order_id = $1', [order_id]);
        if (result.rows.length === 0) {
...
const countShippingCosts = async (copy_id) => {
    try {
		const order_id_result = await pool.query('SELECT order_id FROM BookCopy WHERE copy_id = $1', [copy_id]);
		if(order_id_result.rows.length > 0) {
			const order_id = order_id_result.rows[0].order_id;
        	...
        	if(book_id_result.rows.length > 0){
            	...
            	const total_weight_result = await pool.query('SELECT total_weight FROM BookOrder WHERE order_id = $1', [order_id]);
			...
			console.log('No order is found for given book copy.');
		} 
    } catch (error) {
...
const removeFromOrder = async (order_id, copy_id) => {
    try {
